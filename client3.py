...
def main():
    ...
    if args.mode == 'file' or args.mode == 'stream':
        ...
        ws = MyClient(args.mode, args.audiofile, args.uri + '?%s' % (urllib.parse.urlencode([("content-type", content_type)])) + '?%s' % (urllib.parse.urlencode([("token", args.token)])), byterate=args.rate,
