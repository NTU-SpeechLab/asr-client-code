...
class MyClient(WebSocketClient):
    ...
    def __init__(self, audiofile, url, protocols=None, extensions=None, heartbeat_freq=None, byterate=32000,
        ...
        self.clientLogger = logging.getLogger('client.MyClient')
        self.clientLogger.info('creating an instance of client')
    ...
    def opened(self):
        ...
        def send_data_to_ws():
            if self.send_adaptation_state_filename is not None:
                ...
                self.clientLogger.info("Sending adaptation state from %s" % self.send_adaptation_state_filename)
                try:
        ...
        if response['status'] == 0:
            if 'result' in response:
                ...
                if response['result']['final']:
                    ...
                    self.clientLogger.info("Received block transcription: " + trans)
                    self.final_hyps.append(trans)
                    print >> sys.stderr, '\r%s' % trans.replace("\n", "\\n")
                else:
                    print_trans = trans.replace("\n", "\\n")
                    self.clientLogger.info("Received the partial transcription: " + print_trans)
...
def main():
    ...
    parser.add_argument('-t', '--token', default="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVjZWY4ODJlODcxZWU3MzcyZWU0NWQxZCIsImlhdCI6MTU1OTIwMTg3MiwiZXhwIjoxNTYxNzkzODcyfQ.GsYYw8xOmKYlKKObDBMgUTizzH0cpHk6-AmEUXJmh5s", dest="token", help="User token")
    ...
    clientLogger.info("Processing file " + str(args.audiofile.name))
    ...
    clientLogger.info("Received final audio transcription: " + result.encode('utf-8'))
